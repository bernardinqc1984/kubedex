  ...
  return (
    <main className="mx-auto max-w-6xl space-y-4 px-4 py-8">
      <header className="panel p-5">
        <h1 className="text-2xl font-bold">{world.emoji} {worldTitleLabel(world.id, world.title, language)}</h1>
        <p className="text-slate-400">{worldSubtitleLabel(world.id, world.subtitle, language)}</p>
      ...
      {locked ? <div className="panel p-8 text-center text-slate-400">{world.id === 'world5' ? t('world5UnlockMsg') : t('worldLocked')}</div> : <WorldMap world={world} />}
    </main>
  )
}
